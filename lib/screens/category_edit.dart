...
class _EditCategoryState extends State<EditCategory> {
                  ...
                  child: Column(children: [
                    Row(
                      children: [
                        ...
                        _selectedImage != null && categoryImage.isEmpty
                            ? Container(
                                width: size.width * 0.2,
                                height: size.height * 0.09,
                            ...
                            : Container(),
                        _selectedImage == null && categoryImage.isNotEmpty
                            ? Container(
                                width: size.width * 0.2,
                                height: size.height * 0.09,
                            ...
                            : Container(),
                        _selectedImage != null && categoryImage != ''
                            ? Container(
                                width: size.width * 0.2,
                                height: size.height * 0.09,
                                margin: const EdgeInsets.only(left: 8),
  ...
  handleEdit() async {
    if (_titleController.text.isEmpty) {
    ...
    } else if (_selectedImage == null && categoryImage == '') {
      ...
      try {
        bool res = false;
        if (_selectedImage != null) {
          res = await CategoryResource.updateCategoryWithImage(
              _selectedImage!,
              _titleController.text.trim(),
              widget.category.id.removeAllWhitespace);
        } else {
          res = await CategoryResource.updateCategory(
              _titleController.text.trim(),
              widget.category.id.removeAllWhitespace);
        }
        if (res) {
